...
pub async fn get_plant_demographics(params: RawPathParams, State(pool): State<PgPool>) -> Response {
    ...
    let changed_plants: Vec<PlantId> = match sqlx::query_as(
        r#"SELECT id, date_created, last_modified FROM plants WHERE last_modified >= $1"#,
    )
    .bind(timestamp.naive_utc())
    .fetch_all(&pool)
    .await
    ...
    let response = VerifyClientPlantListResponse {
        ...
        changed_plants: changed_plants.iter().map(|item| item.id).collect(),
    };
